...
def plotResults(
    ...
    if disp is not None:
        ...
        circ2 = patches.Circle(
            (12.5, -2.5), radius=2 * disp, fc="b", ec="None", alpha=0.3
        )
        circ3 = patches.Circle(
            (12.5, -2.5), radius=3 * disp, fc="b", ec="None", alpha=0.1
        )
        ax.add_artist(circ1)
        ax.add_artist(circ2)
        ax.add_artist(circ3)
        ax.text(
            12.5, -2.5, r"$\sigma$", color="w", fontsize=20, ha="center", va="center"
        )
    ...
    if log:
        wandb.log({f"{description}": wandb.Image(fig)}, step=step)
        wandb.log({f"test/{description}_logL": logL}, step=step)
    fig.show()
...
def plotDifferences(orig, data, gmms, avg, l, patch=None):
    ...
    cs = ax.contourf(
        ...
        extent=(-5, 15, -5, 15),
        cmap=plt.cm.Greys,
        vmin=np.arcsinh(pw / 1e-4).min(),
        vmax=np.arcsinh(pw / 1e-4).max(),
    )
    for c in cs.collections:
        c.set_edgecolor(c.get_facecolor())
...
def getBox(coords):
    ...
    return (
        (coords[:, 0] > box_limits[0, 0])
        & (coords[:, 0] < box_limits[1, 0])
        & (coords[:, 1] > box_limits[0, 1])
        & (coords[:, 1] < box_limits[1, 1])
    )
...
def getSelection(type="hole", rng=np.random):
    if type == "hole":
        cb = getHole
        ps = patches.Circle([6.5, 6.0], radius=2, fc="none", ec="k", lw=1, ls="dashed")
    if type == "box":
        cb = getBox
    ...
    if type == "boxWithHole":
        cb = getBoxWithHole
        ps = [
            patches.Circle([6.5, 6.0], radius=2, fc="none", ec="k", lw=1, ls="dashed"),
...
if __name__ == "__main__":
    ...
    gmm.mean[:, :] = (
        np.array(
            [[0.08016886, 0.21300697], [0.70306351, 0.6709532], [0.01087670, 0.852077]]
        )
        * 10
    )
    gmm.covar[:, :, :] = (
        np.array(
            [
                ...
                [[0.00258605, 0.00409287], [0.00409287, 0.01065186]],
            ]
        )
        * 100
    )
    ...
    for r in range(T):
        if bg is not None:
            bg.amp = bg_amp
        l[r], _ = pygmmis.fit(
            gmms[r], data, covar=covar, w=w, cutoff=cutoff, background=bg, rng=rng
        )
    avg = pygmmis.stack(gmms, l)
    print("execution time %ds" % (datetime.datetime.now() - start).seconds)
    plotResults(
        orig, data, avg, patch=ps, description="Standard EM & noise deconvolution"
    )
    ...
    print("execution time %ds" % (datetime.datetime.now() - start).seconds)
    plotResults(
        orig, data, avg, patch=ps, description="$\mathtt{GMMis}$ & noise deconvolution"
    )
