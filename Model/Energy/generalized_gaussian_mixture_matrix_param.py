...
class GeneralizedGaussianMixtureEnergy(Energy):
    ...
    def energy(self, x: torch.Tensor = None) -> torch.Tensor:
        ...
        for k in range(self.num_cluster):
            energy_per_cluster.append(
                ...
                * ((x - self.mu[k]) @ precision_matrix[k] @ (x - self.mu[k]).t()).diag()
                # - torch.nn.functional.log_softmax(self.logit_pi, dim=0)[k]
            )

        self.dependency = torch.min(
            torch.stack(energy_per_cluster, dim=1), dim=1
        ).indices
        # print(torch.bincount(self.dependency))
        ...
        # energy = torch.logsumexp(energy_per_cluster, dim=-1)
        return energy
