...
class KentDistribution(object):
    ...
    def rvs(self, n_sample=None):
        ...
        If n_sample == None then a single sample x is returned with shape (3,)
        If n_sample is an integer value N then N samples are returned in an array with shape (N, 3)
        """
        num_samples = 1 if n_sample == None else n_sample
...
def kent_me(xs):
    ...
    S = average(
        xs.reshape((lenxs, 3, 1)) * xs.reshape((lenxs, 1, 3)), 0
...
def kent_mle(
    ...
    k_me = kent_me(xs)
    gamma1, gamma2, gamma3, kappa, beta = (
        k_me.gamma1,
        k_me.gamma2,
        k_me.gamma3,
        k_me.kappa,
    ...
    def generate_k(fudge_kappa, fudge_beta):
        ...
        return kent2(
            gamma1, gamma2, gamma3, min_kappa + abs(fudge_kappa), abs(fudge_beta)
        )
